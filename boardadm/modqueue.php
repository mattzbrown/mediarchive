...
<tr><td COLSPAN=6 align=center><font SIZE=6>Moderation Queue</font></td></tr>";
...
echo "</i></font></td></tr>";
$sql="SELECT modby FROM modded ORDER BY modid DESC LIMIT 0,1";
$result=mysql_query($sql);
$myrow=@mysql_fetch_array($result);
$sql="SELECT username FROM users WHERE userid=".$myrow["modby"];
$result=mysql_query($sql);
$myrow=@mysql_fetch_array($result);
if ($myrow["username"]) { echo "<tr><td class=\"DARK\" colspan=\"6\" align=\"center\">Last Moderation By: ".$myrow["username"]."</td></tr>"; }
echo "<tr>
<td class=\"LITE\"><i>ID</i></td>
<td class=\"LITE\" width=50%><i>Topic</i></td>
<td class=\"LITE\"><i>Board</i></td>
<td class=\"LITE\"><i>Reason</i></td>
<td class=\"LITE\"><i>Number of Marks</i></td>
<td class=\"LITE\"><i>Date Marked</i></td>
</tr>
";
if ($level==50)  {
$modstr=" ";
...
$marks=mysql_num_rows($result3);
$sql="SELECT * FROM secmarked WHERE markid=$markid ORDER BY marksec DESC LIMIT 0,1";
$result3=mysql_query($sql);
...
echo "<tr>
<td CLASS=CELL1><a href=modact.php?markid=".$myrow["markid"]."";
if ($board) {echo "&board=$board"; }
if ($topic) {echo "&topic=$topic"; }
echo ">".$myrow["markid"]."</a></td>
<td CLASS=CELL1>".stripslashes($myrow2["topicname"])."</td>
<td CLASS=CELL1>".$boardn."</td>
<td CLASS=CELL1>".$myrowre["ruletitle"]."</td>
<td CLASS=CELL1>".$marks."</td>
<td CLASS=CELL1>".$myrow["markdate"]."</td>
</tr>";
}
} else if ($level==52) {
$modstr=" ";
...
$modstr=implode(" OR ",$modstr);
$modstr2="WHERE ".$modstr;
$sql="SELECT * FROM marked ".$modstr2." ORDER BY markid ASC";
$result=mysql_query($sql);
while ($myrow=@mysql_fetch_array($result)) {
$boardid=$myrow["board"];
$topicid=$myrow["topic"];
...
$sql="SELECT * FROM marked ".eregi_replace("=","<>",eregi_replace("OR","AND",$modstr2))." ORDER BY markid ASC";
$result=mysql_query($sql);
while ($myrow=mysql_fetch_array($result)) {
$boardid=$myrow["board"];
$topicid=$myrow["topic"];
...
$sql="SELECT * FROM marked ORDER BY markid ASC";
$result=mysql_query($sql);
while ($myrow=mysql_fetch_array($result)) {
$boardid=$myrow["board"];
$topicid=$myrow["topic"];
