<?
$pagetitle="Moderated Message History";
...
if (!$myrow) {
echo "<tr><td align=center><b>There was an error viewing user moderations:</b> This user does not exist.</td></tr>
<tr><td><br>
<table border=1 cellspacing=0 width=80% align=center>
<form action=\"modhist.php";
if ($board) {echo "?board=$board"; }
...
$sql="SELECT * FROM modded WHERE moduser=$user AND action>0 ORDER BY modid DESC";
$result=mysql_query($sql);
...
echo "<tr><td CLASS=CELL2><a href=\"moddetl.php?user=$user&modid=".$myrow["modid"]."";
if ($board) { echo "&board=$board"; }
if ($topic) { echo "&topic=$topic"; }
...
if (($myrow["contest"]==0) && ($myrow["recont"]==0) && (!$contbo)) {
echo "N/A";
} else if (($myrow["contest"]==0) && ($contbo)) {
echo "Accepted";
