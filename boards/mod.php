...
echo $harsss."<table border=\"0\" width=\"100%\">\n<tr><td align=\"center\"><b><span style=\"font-size:250%;\">Message Board Moderation</span></b></td></tr>";
...
if ($u["level"] < LEVEL_NEW1) {
	echo "\n<tr><td>You are not authorized to mark messages.</td></tr>\n</table>";
	include("/home/mediarch/foot.php");
	exit;
}
$m=@mysql_fetch_array($f);
$ub = @mysql_fetch_array(mysql_query("SELECT `userid` FROM `users` WHERE `username` = '".$m["messby"]."'"));
$sse = mysql_query("SELECT `ruletitle` FROM `tos` WHERE `markshow` = 1 AND `tosid` = ".$reason);
$tos = @mysql_fetch_array($sse);
$sug2 = @mysql_num_rows(mysql_query("SELECT `secmarkid` FROM `secmarked` WHERE `message` = ".$message." AND `markby` = ".$u["userid"]));
$first = FALSE;
$last = FALSE;
...
if ($u["userid"] == $ub["userid"]) {
	echo "\n\n<meta http-equiv=\"refresh\" content=\"10; URL=genmessage.php?board=".$board."&topic=".$topic."\" />\n\n<tr><td>You cannot mark your own messages. You will be returned to the <a href=\"genmessage.php?board=".$board."&topic=".$topic."\">General Message List</a> in ten seconds.</td></tr>\n</table>";
} else if ($u["userid"] != $ub["userid"]) {
	if (@mysql_num_rows($sse) <= 0) {
		echo "\n\n<meta http-equiv=\"refresh\" content=\"10; URL=genmessage.php?board=".$board."&topic=".$topic."\" />\n\n<tr><td>You did not select a reason to mark this message. You will be returned to the <a href=\"genmessage.php?board=".$board."&topic=".$topic."\">General Message List</a> in ten seconds.</td></tr>\n</table>";
	} else if (($tos["ruletitle"] == "Other") && (!$reasontext)) {
		echo "\n\n<meta http-equiv=\"refresh\" content=\"10; URL=genmessage.php?board=".$board."&topic=".$topic."\" />\n\n<tr><td>You did not input an &quot;other&quot; reason to mark this message. You will be returned to the <a href=\"genmessage.php?board=".$board."&topic=".$topic."\">General Message List</a> in ten seconds.</td></tr>\n</table>";
	} else {
		if (($u["ignore"] <= 0) && ($sug2 <= 0)) {
			if ($tos["ruletitle"] != "Other") $reasontext="";
			$sug = @mysql_fetch_array($sugf);
			if (@mysql_num_rows($sugf) <= 0) {
				mysql_query("INSERT INTO `marked` (`reason`,`markwho`,`message`,`topic`,`board`,`reason2`,`markby`,`marksec`) VALUES (".$reason.",".$ub["userid"].",".$message.",".$topic.",".$board.",'".$reasontext."',".$u["userid"].",".time().")");
				$sug = @mysql_fetch_array(mysql_query("SELECT `markid` FROM `marked` WHERE `message` = ".$message));
				mysql_query("INSERT INTO `secmarked` (`markid`,`reason`,`markwho`,`message`,`topic`,`board`,`reason2`,`markby`,`marksec`) VALUES (".$sug["markid"].",".$reason.",".$ub["userid"].",".$messageid.",".$topic.",".$board.",'".$reasontext."',".$u["userid"].",".time().")");
			} else if (@mysql_num_rows($sugf) >= 1) {
				mysql_query("UPDATE `marked` SET `markwho` = ".$ub["userid"].", `reason` = ".$reason.", `markby` = ".$u["userid"].", `marksec` = ".time().", `reason2` = '".$reasontext."' WHERE `markid` = ".$sug["markid"]);
				mysql_query("INSERT INTO `secmarked` (`markid`,`reason`,`markwho`,`message`,`topic`,`board`,`reason2`,`markby`,`marksec`) VALUES (".$sug["markid"].",".$reason.",".$ub["userid"].",".$messageid.",".$topic.",".$board.",'".$reasontext."',".$u["userid"].",".time().")");
			}
		}
	echo "\n\n<meta http-equiv=\"refresh\" content=\"10; URL=genmessage.php?board=".$board."&topic=".$topic."\" />\n\n<tr><td>Your markion notice has been accepted. Thank you for making the ".$sitetitle." message boards a better place! You will be returned to the <a href=\"genmessage.php?board=".$board."&topic=".$topic."\">General Message List</a> in ten seconds.</td></tr>\n</table>";
	}
}
